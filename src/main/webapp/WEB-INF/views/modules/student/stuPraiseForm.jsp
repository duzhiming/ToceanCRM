...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/student/stuPraise/">表扬列表</a></li>
		<li class="active"><a href="${ctx}/student/stuPraise/form?id=${stuPraise.id}">表扬<shiro:hasPermission name="student:stuPraise:edit">${not empty stuPraise.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="student:stuPraise:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="stuPraise" action="${ctx}/student/stuPraise/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="praisedate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					value="<fmt:formatDate value="${stuPraise.praisedate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">表扬的方式：</label>
			<div class="controls">
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="bookdate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					value="<fmt:formatDate value="${stuPraise.bookdate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="confirmdate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					value="<fmt:formatDate value="${stuPraise.confirmdate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">批准：</label>
			<div class="controls">
		...
		<div class="form-actions">
			<shiro:hasPermission name="student:stuPraise:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
